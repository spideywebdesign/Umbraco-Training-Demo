angular.module('umbraco').controller('MyAwesomeSection.EvolvePokemon.Controller', function ($scope, $http, notificationsService, dialogService) {
    ...
    $scope.pokemon = null;
    $http.get('/umbraco/backoffice/api/DemoSectionPokemon/GetPokemon?pokemonId=' + pokemonId).success(function (response) {
        $scope.pokemon = response;
    });

    $scope.evolve = function () {

        $http.get('/umbraco/backoffice/api/DemoSectionPokemon/Evolve?pokemonId=' + pokemonId).success(function () {
